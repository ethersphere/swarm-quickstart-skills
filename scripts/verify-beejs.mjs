...
ok(
  "SUPPORTED_BEE_VERSION is 2.8.x",
  B.SUPPORTED_BEE_VERSION.startsWith("2.8"),
  B.SUPPORTED_BEE_VERSION,
);
...
for (const fn of utilsExpected)
  ok(`Utils.${fn} exists`, typeof B.Utils?.[fn] === "function");
...
for (const fn of goneNames)
  ok(
    `old name '${fn}' is absent`,
    B[fn] === undefined && B.Utils?.[fn] === undefined,
  );
ok(
  "stamp helpers are NOT top-level exports",
  B.getStampCost === undefined && B.getStampEffectiveBytes === undefined,
);
...
ok(
  "getStampEffectiveBytes(17) ~ 40 KB (NOT ~7 MB)",
  d17 > 30_000 && d17 < 50_000,
  `${d17} bytes`,
);
ok(
  'getStampEffectiveBytes(18) ~ 6 MB (the old "7 MB" was depth 18)',
  d18 > 5e6 && d18 < 7.5e6,
  mb(d18),
);
const caps = [17, 18, 19, 20, 21, 22].map((d) =>
  B.Utils.getStampEffectiveBytes(d),
);
ok(
  "effective capacity is monotonic increasing 17->22",
...
ok(
  "capacityBreakpoints depth 20 ~ 680 MB",
  /6\d\d.* MB/.test(eff(20) || ""),
  eff(20),
);
ok(
  "capacityBreakpoints depth 22 ~ 7.7 GB",
  /7\.\d+ GB/.test(eff(22) || ""),
  eff(22),
);
...
for (const x of [
  ...
  ok(`export ${x} exists`, typeof B[x] !== "undefined");

// --- #27: ACT — getNodeAddresses (publisher key source) + Optional unwrap ---
ok(
  "Bee.getNodeAddresses exists (source of publisher PublicKey)",
  typeof B.Bee.prototype.getNodeAddresses === "function",
);
...
for (const m of methods)
  ok(`Bee.${m} exists`, typeof B.Bee.prototype[m] === "function");
...
try {
  ...
  ok(
    "subscribe handler types declare onClose as required (no `?`)",
    /onClose:\s*\(/.test(dts) && !/onClose\?:/.test(dts),
  );
} catch (e) {
...
try {
  ...
  ok(
    "Optional.getOrThrow() unwraps (used for historyAddress)",
    !!opt && typeof opt.getOrThrow === "function" && opt.getOrThrow() === "x",
  );
  ok(
    "Optional exposes .value (alternate unwrap)",
    !!opt && "value" in opt && opt.value === "x",
  );
} catch (e) {
